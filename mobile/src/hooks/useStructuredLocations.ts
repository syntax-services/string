...
export const areaCoordinates: Record<string, { lat: number; lng: number }> = {
  ...
  igan: { lat: 6.932, lng: 3.9238 },
  imere: { lat: 6.931, lng: 3.9246 },
  ...
  itamerin: { lat: 6.945, lng: 3.92 },
  oru: { lat: 6.962, lng: 3.94 },
  'mini-campus': { lat: 6.92, lng: 3.9 },
};
...
export function formatStructuredLocation(selection: StructuredLocationSelection | null): string {
  if (!selection) return '';
  if (!selection.landmark) {
